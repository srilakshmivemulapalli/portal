...
public class QuestionariesRestService {
	...
	public ResponseEntity<ServiceStatusDto> updateQuestionaries(@RequestBody AddQuestionDTO questionDTO)throws QuestionariesServiceException{
		logger.info("QuestionariesRestService *****:: updateQuestionaries"+questionDTO.getEmailId()+"-"+questionDTO.getQuestionId() +"-"+questionDTO.getCategoryId()+"-"+questionDTO.getQuestion()+"-"+questionDTO.getQuestion());
		ServiceStatusDto serviceStatusDto=new ServiceStatusDto();
		serviceStatusDto.setStatus(true);
		serviceStatusDto.setMessage(Constants.MSG_RECORD_UPDATE);
		logger.info("in side service ********serice..");
		try {
			String question = questionariesService.updateQuestion(questionDTO.getQuestionId(),questionDTO.getCategoryId(),questionDTO.getQuestion(),questionDTO.getDescription(),questionDTO.getEmailId());
			if (question != null) {
			return new ResponseEntity<ServiceStatusDto>(serviceStatusDto,HttpStatus.OK);
			}else {
